...
g_classes = [
    x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))
]
...
def room2blocks(data,
    ...
    if not random_sample:
        ...
        num_block_y = int(
            np.ceil(collect_point_label(limit[1] - block_size) / stride)) + 1
...
def bbox_label_to_obj(input_filename, out_filename_prefix, easy_view=False):
    ...
    for i in range(bbox.shape[0]):
        if easy_view and (label[i] not in g_easy_view_labels):
            continue
        obj_filename = out_filename_prefix + '_' + g_classes[label[
            i]] + '_' + str(ins_cnt) + '.obj'
        mtl_filename = out_filename_prefix + '_' + g_classes[label[
            i]] + '_' + str(ins_cnt) + '.mtl'
