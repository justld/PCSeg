...
def auc_roc(logits, label, num_classes, ignore_index=None):
    ...
    Args:
        logits (Tensor): The prediction by model on testset, of shape (N,C,num_points) .
        label (Tensor): The ground truth of image.   (N,1,num_points)
        num_classes (int): The unique number of target classes.
        ignore_index (int): Specifies a target value that is ignored. Default: 255.
    ...
    if len(label.shape) != 3:
        raise ValueError(
            'The shape of label is not 4 dimension as (N, C, num_points), it is {}'.
            format(label.shape))

    if len(logits.shape) != 3:
        raise ValueError(
            'The shape of logits is not 4 dimension as (N, C, num_points), it is {}'.
            format(logits.shape))

    N, C, num_points = logits.shape
    logits = np.transpose(logits, (1, 0, 2))
    logits = logits.reshape([C, N * num_points]).transpose([1, 0])

    label = np.transpose(label, (1, 0, 2))
    label = label.reshape([1, N * num_points]).squeeze()
